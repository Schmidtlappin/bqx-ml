...
BEGIN
    ...
    LOOP
        EXECUTE format('CREATE TABLE IF NOT EXISTS bqx.market_regime_bqx_%I (
...
BEGIN
    ...
    LOOP
        FOR year IN 2024..2025 LOOP FOR month IN 1..12 LOOP
            ...
            EXECUTE format('CREATE TABLE IF NOT EXISTS bqx.market_regime_bqx_%s_%s_%s
            PARTITION OF bqx.market_regime_bqx_%I FOR VALUES FROM (%L) TO (%L)',
            pair_name, year, lpad(month::TEXT, 2, '0'), pair_name,
            make_date(year, month, 1), make_date(year, month, 1) + INTERVAL '1 month');
            partition_count := partition_count + 1;
        END LOOP; END LOOP;
    END LOOP;
    RAISE NOTICE '✅ Created % market_regime_bqx partitions', partition_count;
END $$;
COMMIT;
\echo '✅ Stage 1_9_4 Complete: market_regime_bqx created (10 features, 672 partitions)'
